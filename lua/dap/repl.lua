...
local history = {last = nil, entries = {}, idx = 1}

local autoscroll = vim.fn.has('nvim-0.7') == 1

local function get_session() return require('dap').session() end
...
local function new_buf()
    ...
    if ok then api.nvim_buf_set_option(buf, 'path', path) end
    ...
    api.nvim_buf_set_keymap(buf, 'n', 'o',
                            ...
                            {})
    api.nvim_buf_set_keymap(buf, 'i', '<up>',
                            "<Cmd>lua require('dap.repl').on_up()<CR>", {})
    api.nvim_buf_set_keymap(buf, 'i', '<down>',
                            "<Cmd>lua require('dap.repl').on_down()<CR>", {})
...
local function new_win(buf, winopts, wincmd)
    assert(not wincmd or type(wincmd) == 'string',
           'wincmd must be nil or a string')
...
local repl = ui.new_view(new_buf, new_win, {
    before_open = function() return api.nvim_get_current_win() end,
    after_open = function(_, prev_win) api.nvim_set_current_win(prev_win) end
})
...
function M.print_stackframes(frames)
    if not repl.buf then return end
    local session = get_session()
    frames = frames or
                 (session and session.threads[session.stopped_thread_id] or {}).frames or
                 {}
    local context = {}
    ...
    context.actions = {
        {
            ...
            fn = function(layer, frame)
                ...
                if s then
                    s:_frame_set(frame)
                    layer.render(frames, render_frame, context, start,
                                 start + #frames)
                else
                    utils.notify(
...
local function evaluate_handler(err, resp)
    if err then
        ...
        if message then M.append(message) end
        return
    end
    local layer = ui.layer(repl.buf)
...
local function print_scopes(frame)
    ...
    local layer = ui.layer(repl.buf)
    for _, scope in pairs(frame.scopes or {}) do tree.render(layer, scope) end
end
...
local function print_threads(threads)
    ...
    local tree = ui.new_tree(spec)
    local layer = ui.layer(repl.buf)
    local root = {id = 0, name = 'Threads', threads = vim.tbl_values(threads)}
...
function M.on_up() select_history(-1) end

function M.on_down() select_history(1) end

---@param line string
---@param lnum (integer|string)?
function M.append(line, lnum)
    local buf = repl._init_buf()
    if api.nvim_get_current_win() == repl.win and lnum == '$' then lnum = nil end
    ...
    if autoscroll and repl.win and api.nvim_win_is_valid(repl.win) then
        api.nvim_win_set_cursor(repl.win, {lnum + 2, 0})
    end
    return lnum
end
...
do
    ...
    function M.omnifunc(findstart, base)
        ...
        if vim.startswith(line_to_cursor, '.') or base ~= '' then
            if findstart == 1 then return offset end
            local completions = {}
        ...
        local supportsCompletionsRequest =
            ((session or {}).capabilities or {}).supportsCompletionsRequest;
