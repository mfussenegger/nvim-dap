...
function Session:connect(host, port, opts)
  ...
  client:read_start(rpc.create_read_loop(function(body) o:handle_body(body) end))
  return o
end
...
function Session:event_terminated()
  ...
  -- TODO: Ugly hax
  --    Could probably do this with custom event handlers?
  require('dap')._set_current_session(nil)
end
...
function Session:set_breakpoints(bufexpr, on_done)
  local bp_signs = signs.get_breakpoints(bufexpr)
        ...
        if err1 then
          ...
          for _, bp in pairs(resp.breakpoints) do
            if not bp.verified then
              ...
              signs.remove_breakpoints(bufnr, bp.line)
            end
          end
        end
        num_bufs = num_bufs - 1
